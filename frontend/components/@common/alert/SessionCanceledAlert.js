...
const SessionCanceledAlert = ({
  data,
  showDeleteModalInfoHandale,
  isTutor,
  isStudent,
  isParent,
}) => {
        ...
        <div className="flex-fill vstack gap-2">
          <div className="d-flex gap-2 justify-content-between">
            <p className="alert-heading">
              {t("NotificationsTab.SessionCancelAlertTitle")}
            </p>
            <button
          ...
          <div className="d-flex flex-sm-row flex-column gap-2 justify-content-between mt-auto">
              ...
              <span>
                {isTutor
                  ? data?.sender?.role_id?.id === 3
                    ? t(
                        "NotificationsTab.SessionCancelAlertSubTitleForTutorInParent"
                      )
                    : t("NotificationsTab.SessionCancelAlertSubTitleForTutor")
                  : isStudent
                  ? data?.sender?.role_id?.id === 3
                    ? t(
                        "NotificationsTab.SessionCancelAlertSubTitleForTutorInParent"
                      )
                    : t("NotificationsTab.SessionCancelAlertSubTitleForStudent")
                  : data?.sender?.role_id?.id === 2
                  ? t("NotificationsTab.SessionCancelAlertSubTitleForParent")
                  : t("NotificationsTab.SessionCancelAlertSubTitleForTutor")}
              </span>
            ...
            <div
              ...
              onClick={() => showDeleteModalInfoHandale(data)}
            >
...
export default SessionCanceledAlert;
