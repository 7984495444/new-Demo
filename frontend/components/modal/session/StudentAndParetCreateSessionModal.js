...
import {
  ...
  getAllParentStudentSbjectToTutorIdAction,
  getAllStudentTutorAction,
...
import { getAllParentStudentSbjectAction } from "../../../redux/actions/subjectAction";
import { getAllParentStudentAction } from "../../../redux/actions/tutorAction";
...
const StudentAndParetCreateSessionModal = ({
  userData,
  isStudent,
  show,
  hide,
  ...
  useEffect(() => {
    if (userData?.role_id?.id === 4) {
      dispatch(getAllStudentTutorAction()); // call only tutor
    } else if (userData?.role_id?.id === 3) {
      dispatch(getAllParentStudentAction());
    }
  }, []);
  ...
  useEffect(() => {
    if (sessionInfo?.student) {
      if (userData?.role_id?.id === 3) {
        dispatch(getAllParentStudentSbjectAction(sessionInfo?.student));
      } else {
        dispatch(getAllParentStudentSbjectAction(sessionInfo?.student));
      }
    }
  ...
  const {
    allStudentTutor,
    allParentStudent,
    allParentStudentSubjectToTutorId,
  } = useSelector((state) => state.tutor);

  const { allSubjectByStudentAndTutorId, allParentStudentSubject } =
    useSelector((state) => state.subject);

  useEffect(() => {
    let Options = [];
    let data = userData?.role_id?.id === 3 ? allParentStudent : allStudentTutor;
  ...
  }, [allStudentTutor, allParentStudent]);

  const onChangeHandle = (field, value) => {
    if (field === "subject") {
      dispatch(
        getAllParentStudentSbjectToTutorIdAction(sessionInfo?.student, value)
      );
    }
    setSessionInfo({
      ...sessionInfo,
  ...
  const addSessionHandle = () => {
    ...
    if (date && hour && subject && duration && student) {
      ...
      // let hours2 = moment(convertToUniversalTime(date, new Date())).format("H");
      ...
      let fields = {
        ...
        receiver_id:
          userData?.role_id?.id === 3
            ? allParentStudentSubjectToTutorId?.id
            : Number(student),
        user_id:
          userData?.role_id?.id === 3
            ? allParentStudentSubjectToTutorId?.id
            : Number(student),
        student_id:
          userData?.role_id?.id === 3 ? Number(student) : userData?.id,
        old_session_details: {
          session_date: `${moment(date).format("YYYY-MM-DD")} ${moment(
            hour
          ).format("HH:mm")}`,
      ...
      dispatch(
        addTutorSessionAction(fields, sendNotificationField, null, null, false)
      );
      hide();
    } else {
      let formErrors = addSessionValidation(sessionInfo, true, userData);
  ...
  const onchangeData = (e, data) => {
    if (userData?.role_id?.id === 3) {
      ...
      dispatch(getAllSubjectByStudentIdAction(data.value, userData?.id));
    }
      ...
      <ModalBody className="px-5 py-3">
        <Form>
          <Row className="gy-6">
            ...
            <Col sm="6">
              <Label>
                {isStudent ? t("NewSession.Tutor") : t("NewSession.Student")}
              </Label>
              <Dropdown
                placeholder={isStudent ? "Select tutor" : "Select Élève"}
                closeOnChange={true}
                className="user-list-dropdown form-control-plaintext custom-input-1 border-light-blue-b"
                icon={
                  <ArrowDown2 size="20" className="user-list-dropdown-caret" />
                }
            ...
            <Col sm="12">
              ...
              >
                ...
                {userData?.role_id?.id === 3
                  ? allParentStudentSubject &&
                    allParentStudentSubject?.map((val, index) => {
                      return (
                        <option
                          value={val.id}
                          key={index}
                          selected={sessionInfo?.subject}
                        >
...
export default StudentAndParetCreateSessionModal;
